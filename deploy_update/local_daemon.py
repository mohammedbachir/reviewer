...
def enrich_business(biz):
    ...
    try:
        ...
        snapshots = []
        if biz.get("id"):
            snapshots = _get_snapshots(biz["id"])
...
def run_once():
    ...
    for biz in businesses:
        ...
        try:
            ...
            try:
                resp = cffi_requests.get(
                    f"{SUPABASE_URL}/rest/v1/businesses?select=id&name=eq.{result['name']}&city=eq.{city}&sector=eq.{sector}&limit=1",
                    headers={"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"},
                    timeout=5,
                )
                rows = resp.json()
                if rows:
                    _save_snapshot(rows[0]["id"], result)
            except Exception:
                pass

            emails = 1 if result.get("email") else 0
            temp = result.get("lead_temperature", "COLD")
            crisis_level = result.get("crisis_risk_level", "UNKNOWN")
            crisis_prob = result.get("crisis_probability", 0)
            source = result.get("source", "?")
            print(f"  [{temp}] {result.get('name', '?')[:35]} (src:{source})")
            if result.get("email"):
                print(f"  Email: {result.get('email')}")
            print(f"  Health: {result.get('health_score', 0)} | Vulns: {len(result.get('vulnerabilities', []))}")
            print(f"  Crisis: {crisis_level} ({crisis_prob*100:.1f}%) | CVSS: {result.get('cvss_severity', 'NONE')}")
        except Exception as e:
            log.error(f"  Enrich failed for {biz.get('name','?')}: {e}")

    elapsed = time.time() - t0
    print(f"  Done: {len(results)}/{len(businesses)} in {elapsed:.1f}s")

    state = load_state()
    state["total_runs"] = state.get("total_runs", 0) + 1
    state["total_businesses"] = state.get("total_businesses", 0) + len(results)
    state["total_emails"] = state.get("total_emails", 0) + sum(1 for r in results if r.get("email"))
